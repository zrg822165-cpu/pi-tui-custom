...
export class CombinedAutocompleteProvider {
            ...
            if (spaceIndex === -1) {
                ...
                const commandItems = this.commands.map((cmd) => {
                    ...
                    return {
                        ...
                        ...("argumentChoices" in cmd && Array.isArray(cmd.argumentChoices) && cmd.argumentChoices.length > 0
                            ? { argumentChoices: [...cmd.argumentChoices], argumentIndex: 0 }
                            : {}),
                    };
                });
                const filtered = fuzzyFilter(commandItems, prefix, (item) => item.name).map((item) => ({
                    ...
                    ...(item.argumentChoices ? { argumentChoices: item.argumentChoices, argumentIndex: item.argumentIndex ?? 0 } : {}),
                }));
    ...
    applyCompletion(lines, cursorLine, cursorCol, item, prefix) {
        ...
        if (isSlashCommand) {
            ...
            const argumentValue = Array.isArray(item.argumentChoices) && item.argumentChoices.length > 0
                ? item.argumentChoices[Math.max(0, Math.min(item.argumentIndex ?? 0, item.argumentChoices.length - 1))]
                : undefined;
            const commandText = argumentValue ? `/${item.value} ${argumentValue}` : `/${item.value}`;
            const suffix = argumentValue ? "" : " ";
            const newLine = `${beforePrefix}${commandText}${suffix}${adjustedAfterCursor}`;
            const newLines = [...lines];
            newLines[cursorLine] = newLine;
            return {
                lines: newLines,
                cursorLine,
                cursorCol: beforePrefix.length + commandText.length + suffix.length,
            };
        }
