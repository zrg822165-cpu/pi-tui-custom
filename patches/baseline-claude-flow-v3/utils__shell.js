...
function findBashOnPath() {
    if (process.platform === "win32") {
        ...
        try {
            const result = runProcessSync("where", ["bash.exe"], { encoding: "utf-8", timeout: 5000 });
    ...
    try {
        const result = runProcessSync("which", ["bash"], { encoding: "utf-8", timeout: 5000 });
        if (result.status === 0 && result.stdout) {
            const firstMatch = result.stdout.trim().split(/\r?\n/)[0];
            if (firstMatch) {
                return firstMatch;
...
export function killProcessTree(pid) {
    if (process.platform === "win32") {
        ...
        try {
            startNodeProcess("taskkill", ["/F", "/T", "/PID", String(pid)], {
                stdio: "ignore",
                detached: true,
            });
        }
