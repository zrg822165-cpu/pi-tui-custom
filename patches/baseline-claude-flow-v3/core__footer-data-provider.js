...
import { runProcessLines, runProcessSync } from "../../../../../shell-executor/index.mjs";
...
function resolveBranchWithGitSync(repoDir) {
    const result = runProcessSync("git", ["--no-optional-locks", "symbolic-ref", "--quiet", "--short", "HEAD"], {
        cwd: repoDir,
        encoding: "utf8",
...
async function resolveBranchWithGitAsync(repoDir) {
    try {
        const result = await runProcessLines("git", ["--no-optional-locks", "symbolic-ref", "--quiet", "--short", "HEAD"], {
            cwd: repoDir,
        });
        if (result.exitCode !== 0) {
            return null;
        }
        const branch = result.stdoutLines.join("\n").trim();
        return branch || null;
    }
    catch {
        return null;
