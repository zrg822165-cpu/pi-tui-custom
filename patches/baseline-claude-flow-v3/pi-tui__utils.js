...
const CLEAN_WIDTH_CACHE_SIZE = 1024;
const cleanWidthCache = new Map();
function rememberCacheValue(cache, maxSize, key, value) {
...
export function visibleWidth(str) {
    ...
    const cleanCached = cleanWidthCache.get(clean);
    if (cleanCached !== undefined) {
        rememberCacheValue(widthCache, WIDTH_CACHE_SIZE, str, cleanCached);
        return cleanCached;
    }
    // Calculate width
    let width = 0;
    ...
    // Cache result
    rememberCacheValue(cleanWidthCache, CLEAN_WIDTH_CACHE_SIZE, clean, width);
    rememberCacheValue(widthCache, WIDTH_CACHE_SIZE, str, width);
    return width;
}
/**
