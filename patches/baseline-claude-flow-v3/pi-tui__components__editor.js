...
export class Editor {
    ...
    invalidateLayoutCache() {
        ...
        this.cachedLayoutLines = undefined;
    }
    ...
    handleInput(data) {
        ...
        if (this.autocompleteState && this.autocompleteList) {
            ...
            if (kb.matches(data, "tui.editor.cursorLeft") || kb.matches(data, "tui.editor.cursorRight")) {
                ...
                if (selected && Array.isArray(selected.argumentChoices) && selected.argumentChoices.length > 0) {
                    this.autocompleteList.handleInput(data);
                    this.tui.requestRender();
                    return;
                }
    ...
    layoutText(contentWidth) {
        const layoutKey = `${this.layoutVersion}|${contentWidth}|${this.state.cursorLine}|${this.state.cursorCol}`;
        if (this.cachedLayoutKey === layoutKey && this.cachedLayoutLines) {
            return this.cachedLayoutLines;
        }
        const layoutLines = [];
        if (this.state.lines.length === 0 || (this.state.lines.length === 1 && this.state.lines[0] === "")) {
            ...
            this.cachedLayoutKey = layoutKey;
            this.cachedLayoutLines = layoutLines;
            return layoutLines;
        }
