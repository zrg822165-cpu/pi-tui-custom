 ...
 */
import { Container, Loader, Text } from "@mariozechner/pi-tui";
...
export class BashExecutionComponent extends Container {
    ...
    constructor(command, ui, excludeFromContext = false) {
        ...
        // Command header
        const header = new Text(`${theme.fg(colorKey, "!")} ${theme.fg(colorKey, theme.bold(command))}`, 1, 0);
        this.contentContainer.addChild(header);
        // Loader
        this.loader = new Loader(ui, (spinner) => theme.fg(colorKey, spinner), (text) => theme.fg("muted", text), `运行中... (${keyText("tui.select.cancel")} 取消)`);
    ...
    updateDisplay() {
        ...
        // Command header
        const statusIcon = this.status === "running" ? "●" : this.status === "error" ? "✕" : this.status === "cancelled" ? "■" : "✓";
        const statusColor = this.status === "running" ? "bashMode" : this.status === "error" ? "error" : this.status === "cancelled" ? "warning" : "success";
        const header = new Text(`${theme.fg(statusColor, statusIcon)} ${theme.fg("bashMode", theme.bold(this.command))}`, 1, 0);
        this.contentContainer.addChild(header);
        // Output
        ...
        if (this.status === "running") {
            ...
            if (hiddenLineCount > 0) {
                if (this.expanded) {
                    ...
                    statusParts.push(`${theme.fg("muted", `... 还有 ${hiddenLineCount} 行`)} (${keyHint("app.tools.expand", "展开")})`);
                }
            }
