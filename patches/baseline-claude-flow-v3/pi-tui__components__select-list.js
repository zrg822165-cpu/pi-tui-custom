...
export class SelectList {
    ...
    render(width) {
        ...
        if (this.filteredItems.length === 0) {
            lines.push(this.theme.noMatch("  没有匹配项"));
            return lines;
        }
    ...
    handleInput(keyData) {
        ...
        // Left/right cycle inline argument choices on the selected item.
        else if (kb.matches(keyData, "tui.editor.cursorLeft") || kb.matches(keyData, "tui.editor.cursorRight")) {
            const selectedItem = this.filteredItems[this.selectedIndex];
            const direction = kb.matches(keyData, "tui.editor.cursorLeft") ? -1 : 1;
            if (selectedItem && Array.isArray(selectedItem.argumentChoices) && selectedItem.argumentChoices.length > 0) {
                const currentIndex = selectedItem.argumentIndex ?? 0;
                selectedItem.argumentIndex = (currentIndex + direction + selectedItem.argumentChoices.length) % selectedItem.argumentChoices.length;
                this.invalidate();
                this.notifySelectionChange();
            }
    ...
    getDisplayValue(item) {
        if (Array.isArray(item.argumentChoices) && item.argumentChoices.length > 0) {
            const argumentIndex = Math.max(0, Math.min(item.argumentIndex ?? 0, item.argumentChoices.length - 1));
            const argumentValue = item.argumentChoices[argumentIndex];
            return `${item.label || item.value} ${argumentValue}`;
        }
